...
document.getElementById('BookDetails').addEventListener("submit", function (e) {
    ...
    if (Number(book.year)  > currentYear) {
        alert('Enter valid Year of Publish');
    } else if (Number(book.copies) < 1) {
        alert('Available copies should be greater than or equal to 1');
    } else {
        let books = getData();
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (Number(newBook.year) > currentYear) {
            alert('Enter valid Year of Publish');
        } else if (Number(newBook.copies) < 1) { 
            alert('Available copies should be greater than or equal to 1');
        } else {
